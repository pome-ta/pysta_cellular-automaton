...
class Life(scene.Node):
  ...
  def game_setup(self):
    # ステージデータ
    self.data = [[randint(0, SEED)==0 for x in range(0, self.COLS)]for y in range(0, self.ROWS)]
  ...
  def next_turn(self):
    _data = [[self.check(x, y) for x in range(0, self.COLS)]for y in range(0, self.ROWS)]
    self.data = _data
